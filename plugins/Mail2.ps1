...
if ($Info.error_count.total -eq 0) {
    $msg.Subject = "Choco Auto Update Completed with no errors"
    $msg.Body = @"
<body>
$($info.stats | Out-String)<br/>
<a href="$UserMessage">Click here to see the full update report</a>
</body>
"@
}
else {
    $context = "with errors "
    $msg.Subject = "Choco Auto Update Completed with $($info.error_count.total) $errors_word"
    $msg.Body = @"
<body>
There was $($Info.error_count.total) $errors_word during package auto update.<br/>
$($info.error_info | Out-String)<br/>
<a href="$UserMessage">Click here to see the full update report</a>
</body>
"@
}
